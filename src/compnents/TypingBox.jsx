...
const TypingBox = () => {
  ...
  const [words, setWords] = useState(() => {
    if (testMode === "word") {
      return randomWords({ exactly: testWords, wordsPerString: 1 });
    }
    return randomWords({ exactly: 300, wordsPerString: 1 });
  });
  ...
  const resetTest = () => {
    ...
    if (testMode === "word") {
      setWords(randomWords({ exactly: testWords, wordsPerString: 1 }));
      setWordSpanRef(emptySpans());
      setCountDown(180);
      setTestTime(180);
    } else {
      setWords(randomWords({ exactly: 300, wordsPerString: 1 }));
      setWordSpanRef(emptySpans());
  ...
  const handleKeyDown = (e) => {
    ...
    if (!wordSpanRef[currWordIndex] || !wordSpanRef[currWordIndex].current) {
      return; // Ensure ref is not null
    }
    ...
    if (e.keyCode === 32) {
      ...
      if (
        wordSpanRef[currWordIndex + 1].current &&
      ...
      if (
        wordSpanRef[currWordIndex + 1].current &&
        wordSpanRef[currWordIndex + 1].current.childNodes[0]
      ) {
  ...
  const resetWordSpanRefClassname = () => {
    ...
    if (wordSpanRef[0].current && wordSpanRef[0].current.childNodes[0]) {
  ...
  }, [testSeconds, testWords, testMode,initialRender ]);
